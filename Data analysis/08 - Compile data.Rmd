...
strat_avgs = read.csv("../Compiled data/stratified_averages.csv")%>%
  dplyr::select(LakeID,Layer,Year,Chla_ugL,Chla_date,
                Temp_C,Temp_date,hypo_depth,DOC_mgL,TP_ugL,TN_ugL,
                buoyancy_freq, TP_date,TN_date,SA_vol_ratio)%>%
...
summer_avgs_wide <- summer_avgs%>%
  ...
  pivot_wider(names_from = Layer, 
              values_from = c(TP_ugL, TP_date, DOC_mgL, DOC_date, DO_mgL, 
                              DO_date, Chla_ugL, Chla_date, strat_Temp_C, 
                              strat_Temp_date, Temp_C, Temp_date, TN_ugL, 
                              TN_date, DO_demand_mgLd, AHOD_mgLd, buoyancy_freq, 
                              areal_anoxia, depth_ratio, VHOD5_mgLd, hypo_depth, 
                              strat_TP_ugL, strat_TN_ugL, strat_TP_date, 
                              strat_TN_date, strat_buoyancy_freq,SA_vol_ratio))
...
climate_plot$Region <- factor(climate_plot$Region, 
                              levels = levels(as.factor(climate_plot$Region))[c(1,3,2)])
...
climate_plot%>%
  ...
  geom_errorbar(aes(ymin = Temp_C - Temp_C_sd, 
                    ymax = Temp_C + Temp_C_sd), 
                position = position_dodge(width=.5))+
  theme_bw()+
  theme(axis.title.x = element_blank(),
        axis.text.x = element_text(
          hjust = ifelse(levels(climate_plot$Month) == "N:     Jan\nS:      Jul", 
                         .8, 
                         .5))) +
