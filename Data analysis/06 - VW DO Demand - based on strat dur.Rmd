...
lake_bats <- read.csv("https://pasta-s.lternet.edu/package/data/eml/edi/1029/4/6af06151bb8785235ec1b55ded19e42c")%>%
  filter(Area_m2>0)%>%
  group_by(LakeID)%>%
...
length(unique(vw_do_demand$LakeID[!is.na(vw_do_demand$DO_demand_mgLd)]))
nrow(vw_do_demand[!is.na(vw_do_demand$DO_demand_mgLd),])

#How many rows get removed by filtering so R2>0.5? 246
removed_by_r2 = vw_do_demand%>%
  filter(r2_vol>.5)
nrow(removed_by_r2[!is.na(removed_by_r2$DO_demand_mgLd),])
nrow(vw_do_demand[!is.na(vw_do_demand$DO_demand_mgLd),])
5385-5139
(5385-5139)/5385
...
good_hod = vw_do_demand%>%
  ...
  dplyr::summarize(n_neg = sum(DO_demand_mgLd<0),
                   ...
                   pct = n_neg/n*100)%>%
  filter(pct<=10)
length(unique(good_hod$LakeID))

vw_do_demand_qaqc = removed_by_pos%>%
  filter(LakeID %in% good_hod$LakeID)

nrow(vw_do_demand_qaqc[!is.na(vw_do_demand_qaqc$DO_demand_mgLd),])
length(unique(vw_do_demand_qaqc$LakeID[!is.na(vw_do_demand_qaqc$DO_demand_mgLd)]))
