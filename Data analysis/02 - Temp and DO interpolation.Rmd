...
```{r}
#import data
to_profiles_all <- read.csv("https://pasta-s.lternet.edu/package/data/eml/edi/1029/6/caf59a118a1490e7cb1a219c55b920f9")
to_profiles_all$Date<-as.Date(to_profiles_all$Date)
lat_long <- read.csv("https://pasta-s.lternet.edu/package/data/eml/edi/1029/6/fadd3eaa25b5fdd1fc4efba70e660579") #Metadata
...
nrow(to_profiles%>%filter(reported_max_depth>3)) #The loss of profiles is not primarily because of shallow lakes
...
unclean_plot = unclean%>%
  ...
  geom_text(aes(x=3.05,y=180,label = paste0(
...
unclean_sd = unclean%>%
  ...
  geom_point(alpha = 0.3, color = "blue")+
  geom_text(aes(x=3.05,y=13,label = paste0(
...
clean_plot = clean%>%
  ...
  geom_hline(yintercept=100)+
  ylim(c(min(unclean$pct_sampled),max(unclean$pct_sampled)))+
...
jpeg("../results/figures/depth_cleaning_all.jpg",res=300,width=6, height = 5, units = "in")
...
database.interp %>% filter(!is.na(Temp_C)) %>% group_by(LakeID) %>% dplyr::select(Date)%>% unique() #18363
