...
selected_drivers <- c("DO_demand_mgLd_HYPO", "spring_temp", "winter_temp", "DO_date_HYPO")
...
mod_lmer <- lmer(DO_mgL_HYPO~DO_demand_mgLd_HYPO+spring_temp+winter_temp+DO_date_HYPO+(1|LakeID), data = std_data) 
vif(mod_lmer)
...
selected_drivers <- c("DO_demand_mgLd_HYPO","spring_temp","Temp_C_HYPO")
...
mod_lmer <- lmer(AF~spring_temp+DO_demand_mgLd_HYPO+Temp_C_HYPO+(1|LakeID), data = std_data) 
mod_lmer
vif(mod_lmer)
...
aic_calculator_lmer(data_log_nuts,responses,potential_drivers, interaction = "+")
selected_drivers <- c("strat_TP_ugL_EPI","AF")
std_data <-standardize_data(data_log_nuts,responses,selected_drivers)
all_lakes_hypoP_af <- mod_by_lake(data_log_nuts,responses,selected_drivers, interaction = "+")
mod_lmer <- lmer(TP_ugL_HYPO~strat_TP_ugL_EPI+AF+(1|LakeID), data = std_data)
vif(mod_lmer)
...
Segmented DO regression (could be cut?)
```{r}
### DO (rather than anoxic factor)
responses <- c("DO_mgL_HYPO")
potential_drivers <- c("DO_demand_mgLd_HYPO",
                      "spring_temp",
                      "summer_temp",
...
responses <- c("AF")
aic_calculator_onevar(for_mlr,responses,potential_drivers)
summary(lm(AF~mean_af, data = for_mlr)) #Take home: nothing comes out cleanly
...
p_value = summary(lm(DO_demand_mgLd_HYPO_af~log_MeanDepth_m, data = for_mlr))$coefficients[,4][2]
if(p_value < 0.001){p_value_text = "< 0.001"}else{p_value_text = paste0("== ",as.character(round(p_value,3)))}
r2_value = summary(lm(DO_demand_mgLd_HYPO_af~log_MeanDepth_m, data = for_mlr))$r.squared
r2_value_text = round(r2_value,2)
