...
data_always_oxic <- dataset %>%
  filter(!is.finite(max(AF, na.rm = T)) || max(AF, na.rm = T) == 0,
         !is.finite(min(DO_mgL_HYPO, na.rm = T)) || min(DO_mgL_HYPO, na.rm = T) >= 1.8)
...
vif(mod_lmer)
hypo_do_oxic <- plot_effects_lmer(mod_lmer,"Hypolimnetic DO (oxic; mg/L)", poster = POSTER) 
hypo_do_oxic_ridge <- plot_effects_by_lake_lmer_ridge(all_lakes_do,"Hypolimnetic DO (oxic; mg/L)",mod_lmer, poster = POSTER)
...
aic_calculator_lmer(data_no_oxic,responses,potential_drivers, interaction = "+") #same results
...
length(unique(data_log_nuts$LakeID[!is.na(data_log_nuts$strat_TN_ugL_EPI)])) #76
length(unique(data_log_nuts$LakeID[!is.na(data_log_nuts$strat_TP_ugL_EPI)])) #368
...
g5 <- ggplotGrob(
  hypo_do_oxic +theme(plot.title = element_text(size = 10),
                         plot.subtitle = element_text(size = 9))+
                   force_panelsizes(rows = unit(4*5/6, "cm")))
...
g5 <- ggplotGrob(hypo_do_oxic_ridge 
                 #+ggtitle("Bottom-water oxygen")
                 +theme(plot.title = element_text(size = 10),
                         plot.subtitle = element_text(size = 9))+
                   force_panelsizes(rows = unit(4, "cm")))
...
Step 5: Generate combined figures - anoxic factor
```{r}
g1 <- ggplotGrob(
...
layout <- rbind(c(1, 2),
                ...
                c(4, 5),
                c(4, 5))
jpeg("../Figures/MLR-lmer/Parameter estimate-all-ridge-af.jpeg", width = 6, height = 5.5, res = 300, units = "in")
grid.arrange(g1,g2,g3,g4,g5, layout_matrix=layout)
dev.off()
```
...
responses <- c("DO_mgL_HYPO")
aic_calculator_onevar(for_mlr,responses,potential_drivers)
summary(lm(DO_mgL_HYPO~mean_do, data = for_mlr)) #Take home: potentially a relationship with DO, but this is not part of the main ABA analysis (we use AF instead)
...
demand_af <- for_mlr%>%
  ...
  geom_smooth(method = "lm", color = "black")+ #, data = . %>% filter(MeanDepth_m > 1))+
  geom_point()+
  scale_x_log10()+
  geom_hline(yintercept = 0)+
  theme_bw()+
  #scale_color_viridis_c(trans = "log", breaks = c(25,50,100,200), name = "Maximum\ndepth (m)")+
  ylab("Coefficient for the\neffect of VHOD on AF")+
  xlab("Mean depth (m)")+
  guides(shape = "none") +
...
do_tp + scale_color_manual(values = c("black","black"))
dev.off()

do_tp_af_all <- with_lags%>%
  filter(!is.na(TP_ugL_HYPO))%>%
  ungroup()%>%
  mutate(label = paste0("n = ", n(), " lake-years\nn = ",
                        length(unique(LakeID))," lakes"))%>%
  ggplot()+
  scale_y_log10()+
  geom_point(aes(x = DO_mgL_HYPO, y=TP_ugL_HYPO, fill = AF), alpha = 0.3, pch = 21, stroke = 0.2)+
  geom_text(aes(x = 2.5, y=5000,label = label),
            data = .%>%dplyr::select(label)%>%unique(), hjust = 0, size = 3.5)+
  xlab("Hypolimnetic DO (mg/L)")+
  ylab("Hypolimnetic TP (µg/L)")+
  scale_fill_viridis_c(na.value = alpha("white", 0), name = "Anoxic\nfactor")+
  geom_vline(xintercept = 1.8, color = "black")+
  theme_bw()

jpeg("../Figures/DO TP colored by AF.jpeg", width = 4, height = 3, res = 300, units = "in")
do_tp_af_all
dev.off()

jpeg("../Figures/DO TP af.jpeg", width = 3, height = 3, res = 300, units = "in")
do_tp_af <- with_lags%>%
  filter(!is.na(TP_ugL_HYPO))%>%
  ungroup()%>%
  mutate(label = paste0("n = ", n(), " lake-years\nn = ",
                        length(unique(LakeID))," lakes"),
         on_other_fig = ifelse(!is.na(AF) & 
                                 AF > 0,
                               "y",
                               "n"))%>%
  ggplot()+
...
af_tp <- with_lags%>%
  mutate(on_other_fig = ifelse(!is.na(TP_ugL_HYPO) & 
                                 !is.na(DO_mgL_HYPO),
                               "y",
                               "n"))%>%
  filter(!is.na(TP_ugL_HYPO),
  ...
  mutate(label = paste0("n = ",n()," lake-years\nn = ",length(unique(LakeID))," lakes"))%>%
  ggplot()+
  scale_y_log10(limits = c(min(with_lags$TP_ugL_HYPO,na.rm=T),
                           max(with_lags$TP_ugL_HYPO,na.rm=T)))+
  #scale_x_sqrt()+
  geom_point(aes(x = AF,y=TP_ugL_HYPO, color = on_other_fig), alpha = 0.2)+
  geom_text(aes(x = 1, y=5000,label = label),
            data = .%>% dplyr::select(label) %>% unique(), 
            hjust = 0, size = 3.5)+
  #geom_smooth(aes(x = AF,y=TP_ugL_HYPO), color = "black", method = "lm", lwd = .8)+
  scale_color_manual(values = c("black","blue"))+
  xlab("Anoxic factor (d)")+
  theme_bw()+
  theme(axis.title.y = element_blank(),
...
jpeg("../Figures/AF and TP.jpeg", width = 6, height = 3, res = 300, units = "in")
ggarrange(do_tp_af, af_tp, align = "h")
dev.off()
```


Step 8: Before and after figure
```{r}
total <- with_temp%>%
