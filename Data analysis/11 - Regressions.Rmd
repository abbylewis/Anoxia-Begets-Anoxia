...
ggarrange(ggarrange(chla+stat_poly_eq(label.x = .1, label.y = 0.99),
                    demand+stat_poly_eq(label.x = .8, data = .%>%filter(MaximumDepth_m<50)), 
                    labels = c("A","B"), widths = c(1,1.3)),
          ggarrange(vhod+stat_poly_eq(label.x = .1, label.y = 0.91),
                    do+stat_compare_means(label.y.npc = 0.91),
                    labels = c("C","D"), widths = c(1,1.3)), 
          nrow = 2)
dev.off()

jpeg("../Figures/Driver analysis.jpg", width = 7, height = 5, res = 300, units = "in")
ggarrange(ggarrange(chla+stat_poly_eq(label.x = .1, label.y = 0.99),
                    vhod+stat_poly_eq(label.x = .1, label.y = 0.91),
                    labels = c("A","B"),
                    label.x = -0.01),
          ggarrange(demand+stat_poly_eq(label.x = .8, data = .%>%filter(MaximumDepth_m<50)),
...
status_data <- with_temp%>%
  ...
  arrange(LakeID,Year)%>%
  filter(!is.na(DO_mgL_HYPO))%>%
  filter(sum(DO_mgL_HYPO<1.4)>=1,
         )%>%
  mutate(anoxic_date = min(Year[DO_mgL_HYPO<1.4]),
         first_date = min(Year),
         end_date = max(Year),
