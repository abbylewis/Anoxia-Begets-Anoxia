...
POSTER=F #Should figures be designed for a poster? Or for the manuscript
...
responses = c("strat_TP_ugL_EPI")
aic_calculator_onevar(for_mlr,responses,potential_drivers)
summary(lm(strat_TP_ugL_EPI~tn_tp_ratio, data = for_mlr)) #Take home: epi P has a positive relationship

jpeg("../Figures/Effect of TP on chl-a.jpg", width = 4, height = 4, res = 300, units = "in")
chla = for_mlr%>%
  filter(!is.na(strat_TP_ugL_EPI))%>%
  #mutate(ratio = ifelse(tn_tp_ratio>44,"P limited",
  #                      ifelse(tn_tp_ratio<12,"N limited","co-limited")))%>%
...
for_mlr%>%
  filter(!is.na(mean_epi_p),
         !is.na(cv_epi_p))%>%
  ggplot(aes(x=mean_epi_p,y=cv_epi_p))+
  geom_point()+
  scale_x_log10()+
  geom_smooth(method = "lm",color = "black")+
  theme_bw()+
...
ggarrange(ggarrange(chla+stat_poly_eq(label.x = .1, label.y = 0.99),
                    demand+stat_poly_eq(label.x = .8, data = .%>%filter(MaximumDepth_m<50)), labels = c("A","B"), widths = c(1,1.3)),
