...
#' 
#' @export
thermo.depth.density <- function(wtr, depths, Smin = 0.1, seasonal=TRUE, index=FALSE, mixed.cutoff=0.1){
  
  if(any(is.na(wtr))){
    return(NaN)
  }
  
  if(diff(range(rLakeAnalyzer::water.density(wtr), na.rm=TRUE)) < mixed.cutoff){
    return(NaN)
  }
